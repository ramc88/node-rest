...
const works = new Queue("works", {
  redis: {
    port: global.config.redis.split(":")[1],
    host: global.config.redis.split(":")[0],
  },
});
const dbInsert = new Queue("dbInsert", {
  redis: {
    port: global.config.redis.split(":")[1],
    host: global.config.redis.split(":")[0],
  },
});
const deleteJobs = new Queue("deleteJobs", {
  redis: {
    port: global.config.redis.split(":")[1],
    host: global.config.redis.split(":")[0],
  },
...
exports.addToQueue = async (queueName, data, opts) => {
  ...
  try {
    ...
    jobOpts.removeOnComplete =
      jobOpts.removeOnComplete === false ? jobOpts.removeOnComplete : true;
    jobOpts.removeOnFail =
      jobOpts.removeOnFail === false ? jobOpts.removeOnFail : true;
